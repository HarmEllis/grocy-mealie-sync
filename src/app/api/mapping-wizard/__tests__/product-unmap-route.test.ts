...
import { POST } from '../products/unmap/route';

describe('mapping wizard product unmap route', () => {
  beforeEach(() => {
  ...
  it('returns 404 when the product mapping does not exist', async () => {
    const response = await POST(new Request('http://localhost/api/mapping-wizard/products/unmap', {
      method: 'POST',
    ...
    expect(body).toEqual({ error: 'Product mapping not found' });
    expect(mockState.deleteWhereCalls).toEqual([]);
    expect(mockState.releaseSyncLock).toHaveBeenCalledTimes(1);
  });

  it('deletes the requested product mapping', async () => {
    ...
    const response = await POST(new Request('http://localhost/api/mapping-wizard/products/unmap', {
      method: 'POST',
      body: JSON.stringify({ id: 'product-map-1' }),
    }));
    const body = await response.json();
